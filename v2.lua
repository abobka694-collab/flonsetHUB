...
-- ХРАНИЛИЩЕ
-- ═══════════════════════════════════════════════════════════════
local GunESP_Data = {} -- [GunObject] = {highlight, billboard, textLabel}
local UpdateThread = nil
local AddedConn = nil
...
-- ═══════════════════════════════════════════════════════════════
-- ПОЛУЧЕНИЕ БАЗОВОЙ ЧАСТИ ПИСТОЛЕТА
-- ═══════════════════════════════════════════════════════════════
...
-- ═══════════════════════════════════════════════════════════════
-- ПРОВЕРКА ЧТО ОБЪЕКТ НЕ В ИНВЕНТАРЕ ИГРОКА
-- ═══════════════════════════════════════════════════════════════
...
local function UpdateGunESP()
    ...
    -- Проходим по всем сохранённым пистолетам
    for obj, data in pairs(GunESP_Data) do
        -- Проверяем что объект всё ещё существует
        if not obj or not obj.Parent or IsInCharacter(obj) then
            RemoveGunESP(obj)
            continue
        end
...
-- ═══════════════════════════════════════════════════════════════
-- ОБРАБОТКА ПОЯВЛЕНИЯ НОВОГО ОБЪЕКТА
-- ═══════════════════════════════════════════════════════════════
...
local function ScanExistingGuns()
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj.Name == "GunDrop" 
            and (obj:IsA("BasePart") or obj:IsA("Model") or obj:IsA("Tool"))
            and not IsInCharacter(obj) then
            CreateGunESP(obj)
        end
    end
end
...
function GunESP:Enable()
    ...
    -- Сканируем существующие пистолеты
    ScanExistingGuns()
    
    -- Слушаем появление новых
